...
Adafruit_NeoPixel strip( NLEDS*NMODULES, LEDSTRIP_DATA_PIN, NEO_GRBW + NEO_KHZ800);
...
int m2_led_counter = m2_led_offset;
int m1_tailSize = 2;
int m2_tailSize = 2;
...
void m2_cwStep(void)
{
  ...
  m2_led_counter --;
  if( m2_led_counter < m2_led_offset) 
    m2_led_counter = (m2_led_offset+NLEDS)-1;
...
void m2_ccwStep(void)
{
  ...
  m2_led_counter ++;
  if( m2_led_counter >= m2_led_offset + NLEDS) m2_led_counter = m2_led_offset;
...
void loop() 
{
  ...
  else
  {
    ...
    // first shutdown every LEDS
    for(i=m2_led_offset; i<m2_led_offset+NLEDS; i++) {
      strip.setPixelColor(i, 0,0,0,0);
    }
...
void m1_lightTail( int firstLed ) {
...
void m2_lightTail( int firstLed ) {
  // light up all the LEDs composing the tail
  int tailIndex;
  for(int j=0; j<m2_tailSize; j++) {
    tailIndex = (firstLed-m2_led_offset) + j;
    tailIndex %= NLEDS;
    strip.setPixelColor(tailIndex+m2_led_offset, 0,0,0, m2_intensity);
  }
